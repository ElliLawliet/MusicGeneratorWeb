...
if majorminorinput == ('major'):
  Ch1n = random.randint(0,11)*2
else:
  Ch1n = random.randint(0,11)*2+1
...
'''#Repeating chords:
for a in range (0,extender-1):
  for b in ChArray:
    ChArray.append(b)
  for c in FinalChRhArray:
    FinalChRhArray.append(c)'''
...
if "m" in ChArray[0]: #Minor Melody
  ...
  #Melody generation:
  for i in range(random.randint(melodyLength-2,melodyLength*3-2)):
    #print("Fn = ",fn) #Debug
...
if sum(FinalMeloRhArray) - melodyLength >= melodyLength/2:
  MaxRh = melodyLength*2
else:
  MaxRh = melodyLength
